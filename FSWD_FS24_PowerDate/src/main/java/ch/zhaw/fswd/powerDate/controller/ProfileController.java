...
public class ProfileController {
    ...
    public void deleteProfileByLoginName(String loginName) {
        User user = userRepository.findById(loginName).orElseThrow(() -> new UserNotFoundException());
    ...
    public void deleteProfileByUUID(String uuid) {
        Profile profile = profileRepository.findById(UUID.fromString(uuid)).orElseThrow(() -> new UUIDNotFoundException());
        User user = userRepository.findByProfile(profile).orElseThrow(() -> new UserNotFoundException());
        Profile p = user.getProfile();
        user.setProfile(null);
        userRepository.save(user);
        profileRepository.delete(p);
